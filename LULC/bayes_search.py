...
def optimiser(model,data,fold_selector,path,directory,baseline):
	...
	if fold_selector != 4:
		...
		y_test_extracted_inverted = add_last_category(scaler2.inverse_transform(y_test_extracted))
	for i in range(0,3): # 3 best models
		...
		if fold_selector != 4:
			...
			y_preds_extracted_inverted.append(add_last_category(scaler2.inverse_transform(y_preds_extracted[i])))
