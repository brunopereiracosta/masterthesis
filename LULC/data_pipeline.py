...
def lag(dataset, lag_parameter):
	lulc = dataset.iloc[:,0:output_neurons].shift(-lag_parameter)#, fill_value=0)
	covariates = dataset.iloc[:,output_neurons:n_feature]
	data = pd.concat([lulc, covariates], axis=1, sort=False)
...
def split_norm_batch(data,fold_selector,nr_folds=5):
	...
	y = np.array(data.values[:,0:output_neurons])
	...
	x_val, y_val = batching_data(dataset=x_val_norm[fold_selector],target=y_val_norm[fold_selector])	
